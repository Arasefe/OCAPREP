package _28.DateTime.ParsingDatesAndTimes;

import java.time.LocalDate;
import java.time.LocalTime;
import java.time.Month;
import java.time.format.DateTimeFormatter;

public class Test3 {
        public static void main(String[] args) {

            DateTimeFormatter f = DateTimeFormatter.ofPattern("MM-dd-yyyy");
            LocalDate date = LocalDate.parse("02-15-2010", f);
            LocalTime time = LocalTime.parse("11:33");

            System.out.println(date);
            System.out.println(time);

            String text = date.format(f);
            System.out.println(text);
            LocalDate parsedDate = LocalDate.parse(text, f);
            System.out.println(parsedDate);

            LocalDate myDate = LocalDate.of(2010, Month.MAY, 5);
            String dateString = f.format(myDate);
            System.out.println(dateString);

            LocalDate anotherDate = LocalDate.parse(dateString, f);
            anotherDate = anotherDate.plusMonths(5).plusDays(5);
            System.out.println(f.format(anotherDate));
        }
    }

